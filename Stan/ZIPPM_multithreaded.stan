// Zero-Inflated Poisson-Poisson mixture model - multithreading
...
functions{
   ...
   * @param log_mu     Log of Detection rate
   *
  ...
  real poispois_lpmf(array[] int y, int n, real lambda, real mu) {
   ...
   /**
   * Return log probability of a zero inflated Poisson-poisson mixture model for a site
   * 
  ...
  real zipp_lpmf(int[] count, int max_n,
    ...
    if(c_max){  // If non-zero max count
      ...
      for (k in c_max:max_n){
        lp[k + 1] = lp[k + 1] + poispois_lpmf(count| k, lambda, p);
      ...
      for (k in c_max:max_n){
        lp2[2, k + 1] = lp2[2, k + 1] + poispois_lpmf(count| k, lambda, p);
  ...
  real partial_sum(int[] site,
    ...
    for (m in start:end){
      lp = lp + zipp_lpmf(count[m] | max_n, lambda, psi, p);
    }
    return lp;
  }
}
...
generated quantities {
  ...
  
  {
    ...
    for (i in 1 : R) {
      ...
      log_lik[i] = zipp_lpmf(y[i,] | K, lambda, psi, p);
          
      // latent N's and expected counts
      vector[K + 1] lp;
      
      for (n in 0 : K) {
        lp[n + 1] = poispois_lpmf(y[i,] | K, lambda, p);
      }
      ...
      if (max(y[i,]) == 0) {
        // Unobserved
        log_p_unobs = log(psi) + poisson_log_lpmf(0 | log(N[i]+1e-9) + logit(p));
